...
# Description
  ...
  -----------------------------------------------------------------------
  Viewport and origin are not taken into account, and the movie must be
  positioned entirely \'on screen\', otherwsie nothing will be drawn.\
  \
  Movie support relies on DirectShow, so you will need to ensure the
  correct drivers are installed. Movies will typically playback fastest
  at their natural size.\
  \
  When playing MPEG or AVI files, the movie is played back at normal
  speed \'behind the scenes\', and DrawMovie simply draws the most recent
  frame. For animated GIFs, the DrawMovie command simply draws the next
  frame of the animation each time it is called, so you need to manage
  your animation timing manually. See the example below for details.\
  \
  See also: [OpenMovie](OpenMovie.htm){.small},
  [CloseMovie](CloseMovie.htm){.small},
  [MoviePlaying](MoviePlaying.htm){.small},
  [MovieWidth](MovieWidth.htm){.small},
  [MovieHeight](MovieHeight.htm){.small}.

  -----------------------------------------------------------------------

# [Example](../2d_examples/DrawMovie.bb)
  ...
  Color 0,255,0\
  \
  ; Next, open the movie file. Feel free to change this to an AVI or MPEG
  file.\
  movie=OpenMovie(\"media/hat.gif\")\
  \
  ; check to see if it loaded okay\
  If movie=0 Then RuntimeError \"Error - Movie not loaded!\"\
  If Not(MoviePlaying(movie)) Then RuntimeError \"Error - Movie not
  playing!\"\
  \
  ...
  \
  ; GIFs have no timing info, and as such will redraw the next frame on
  each call to DrawMovie.\
  ; AVIs and MPEGs do have timing info, and as such will redraw the most
  recent frame on each call to DrawMovie.\
  ; Ergo, this time limiter only has an impact, and is only required for
  GIFs.\
  Repeat\
  ; do nothing\
  ...
  Text 0,100,\"Current Command Syntax: DrawMovie(movie, \" + x + \",\"+
  y + \",\" + w + \",\" + h + \")\"\
  ; Flip the buffers\
  Flip\
  \
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=DrawMovie&ref=comments){target="_blank"}
