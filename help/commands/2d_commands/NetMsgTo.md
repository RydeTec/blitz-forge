...
# Description
  ...
  RecvNetMsg() command - and probably determined the type of message with
  (NetMsgType().\
  \
  The value returned from this command denotes the messages\'s intended
  recipient\'s ID number assigned to them when they were created with
  CreateNetPlayer.\
  \
  You will use NetMsgType(), NetMsgFrom(), and NetMsgData\$() to get
  other important information from the message and act on it.\
  \
...
# [Example](../2d_examples/NetMsgTo.bb)
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=NetMsgTo&ref=comments){target="_blank"}
