...
# Description
  ...
  \
  Whenever you use the Collisions command to enable collisions between
  two different entity types, information is added to the collision list.
  This command clears that list, so that no collisions will be detected
  until the Collisions command is used again.\
  \
...
# [Example](../3d_examples/ClearCollisions.bb)
  ...
  sphere-\>polygon method and slide response\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=ClearCollisions&ref=comments){target="_blank"}
