...
# Description
  ...
  any animation keys set by SetAnimKey can be used in an actual animation
  however this is optional. You may use it to \"bake\" the frames you
  have added previously using SetAnimKey.\
  \
  Returns the animation sequence number added.

  -----------------------------------------------------------------------

# [Example](../3d_examples/AddAnimSeq.bb)
  ...
  Next\
  \
  ;Now we need to add the frames we\'ve just made to the sequence of
  \"film\"!\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=AddAnimSeq&ref=comments){target="_blank"}
