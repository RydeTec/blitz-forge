...
# Description
  ...
  -----------------------------------------------------------------------
  CaptureWorld \'captures\' the properties (position, rotation, scale,
  alpha etc) of each entity in the 3D world.\
  \
  This is then used in conjunction with the
  [RenderWorld](../3d_commands/RenderWorld.htm){.small} tween parameter
  in order to render an interpolated frame between the captured state of
  each entity and the current state of each entity. See the
  [RenderWorld](../3d_commands/RenderWorld.htm){.small} docs for a full
  explanation of render tweening.\
  \
  See also: [RenderWorld](RenderWorld.htm){.small}.

  -----------------------------------------------------------------------

# [Example](../3d_examples/CaptureWorld.bb)
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=CaptureWorld&ref=comments){target="_blank"}
