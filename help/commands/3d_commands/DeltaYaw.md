...
# Description
  ...
  -----------------------------------------------------------------------
  Returns the yaw angle, that src_entity should be rotated by in order to
  face dest_entity.\
  \
  This command can be used to be point one entity at another, rotating on
  the y axis only.\
  \
  See also: [DeltaPitch](DeltaPitch.htm){.small}.

  -----------------------------------------------------------------------

# [Example](../3d_examples/DeltaYaw.bb)
  ...
  RenderWorld\
  \
  Text 0,0,\"Note: Camera view is overhead. The arrow will y-rotate using
  DeltaYaw value.\"\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=DeltaYaw&ref=comments){target="_blank"}
