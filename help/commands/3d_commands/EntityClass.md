...
# Description
  ...
  BSP\
  \
  Note that the command will fail if a valid entity handle is not
  supplied, and will not just return an empty string.

  -----------------------------------------------------------------------

# [Example](../3d_examples/EntityClass.bb)
  ...
  SeedRnd MilliSecs()\
  \
  ; Select a random number between 0 and 7 then create a certain class of
  entity depending on the number selected\
  i=Rand(0,7)\
  Select i\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=EntityClass&ref=comments){target="_blank"}
