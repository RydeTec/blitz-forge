...
# Parameters
  ...
  realtime (optional) - True to modify terrain immediately. False to
  modify terrain when RenderWorld in next called. Defaults to False.
...
# [Example](../3d_examples/ModifyTerrain.bb)
  ...
  \
  terra_size=32 ; initial size of terrain, and no. of grids segments,
  along each side\
  ...
  \
  camera=CreateCamera()\
  PositionEntity camera,(terra_size\*x_scale)/2,50,0 ; position wherever;
  just try and get good view of terrain!\
  ...
  PositionEntity
  marker,marker_x\*x_scale,marker_y#\*y_scale,marker_z\*z_scale\
  \
  RenderWorld\
  \
  Text 0,0,\"Use cursor keys to move marker over the terrain\"\
  Text 0,20,\"Press A or Z to alter height of terrain at marker\'s
  position\"\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=ModifyTerrain&ref=comments){target="_blank"}
