...
# Description
  ...
  \
  An entity must have its EntityPickMode set to a non-0 value value to be
  \'pickable\'.\
  \
  See also: [EntityPick](EntityPick.htm){.small},
  [LinePick](LinePick.htm){.small}, [CameraPick](CameraPick.htm){.small},
  [EntityPickMode](EntityPickMode.htm){.small}.

  -----------------------------------------------------------------------

# [Example](../3d_examples/CameraPick.bb)
  ...
  \
  plane=CreatePlane()\
  EntityPickMode plane,2 ; Make the plane entity \'pickable\'. Use
  pick_geometry mode no.2 for polygon collision.\
  ground_tex=LoadTexture(\"media/Chorme-2.bmp\")\
  EntityTexture plane,ground_tex\
  \
  cube=CreateCube()\
  EntityPickMode cube,2 ; Make the cube entity \'pickable\'. Use
  pick_geometry mode no.2 for polygon collision.\
  ...
  ; In this example, only three things can be picked: the plane, the
  cube, or nothing\
  ...
  Text 0,20,\"Press left mouse button to use CameraPick with mouse
  coordinates\"\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=CameraPick&ref=comments){target="_blank"}
