...
# [Example](../3d_examples/CameraProject.bb)
  ...
  RenderWorld\
  \
  ; If cube is in view then draw text, if not then draw nothing otherwise
  text will be drawn at 0,0\
  If EntityInView(cube,camera)=True\
  \
  ; Use ProjectedX() and ProjectedY() to get 2D coordinates from when
  CameraProject was used.\
  ; Use these coordinates to draw text at a 2D position, on top of a 3D
  scene.\
...
\
Click
[here](http://www.blitzbasic.co.nz/b3ddocs/command.php?name=CameraProject&ref=comments){target="_blank"}
